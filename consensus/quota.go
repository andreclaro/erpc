...
func resolveMinAgreement(groups []*responseGroup, reqs []*common.ConsensusRequiredParticipant, threshold int) (minAgreementOutcome, *responseGroup) {
	...
	// len(top) > 1: multiple quota-satisfying groups tied at maxCount.
	// Before deferring to tie/dispute rules, verify that no non-satisfying group
	...
	// All threshold-meeting groups that outrank the satisfying tie are themselves
	// satisfying (or don't exist). Safe to defer to tie/dispute rules (9/14).
